...
export default function Page() {
  // Fetch menopausal symptoms data
  const { data, isLoading, error } =
    useConditionTreatedDetailsQuery("hormonal-imbalance");
  ...
  const heading = safe(svc?.name) || safe(svc?.title) || "Menopausal Symptoms";
  ...
  const tabs = [
    {
      id: 1,
      path: "HormoneReplacementTherapy",
      label: "Hormone Replacement Therapy",
      enabled:
        !!safe(det?.hormone_replacement_therapy_title) ||
        !!safe(det?.hormone_replacement_therapy_description) ||
        !!safe(det?.hormone_replacement_therapy_image),
    },
    {
      id: 2,
      path: "CO2",
  ...
  const cardBlocks = [
    (safe(det?.hormone_replacement_therapy_title) ||
      safe(det?.hormone_replacement_therapy_description) ||
      safe(det?.hormone_replacement_therapy_image)) && {
      id: "HormoneReplacementTherapy",
      image: url(det?.hormone_replacement_therapy_image) || FallbackImg.src,
      title:
        safe(det?.hormone_replacement_therapy_title) ||
        "Revitalize Health with Hormone Therapy",
      description:
        safe(det?.hormone_replacement_therapy_description) ||
        "Hormone Replacement Therapy restores hormonal balance and boosts vitality, energy, and mood.",
