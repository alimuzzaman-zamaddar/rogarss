...
export default function page() {
  const { data, isLoading } = useConditionTreatedDetailsQuery("skin-tags");

  const sectionBars = [
    { id: 4, path: "SkinTagRemoval", label: "Skin Tag Removal" },
      ...
      {!isLoading && data && (
        <>
          ...
          <div id="SkinTagRemoval">
            <DynamicCardSection
              image={`${process.env.NEXT_PUBLIC_ASSET_URL}/${data?.data?.subConditionTreats?.sub_condition_treat_details?.skin_tag_removal_image}`}
              title={
                data?.data?.subConditionTreats?.sub_condition_treat_details
                  ?.skin_tag_removal_title
              }
              description={
                data?.data?.subConditionTreats?.sub_condition_treat_details
                  ?.skin_tag_removal_description
              }
            />
          </div>
