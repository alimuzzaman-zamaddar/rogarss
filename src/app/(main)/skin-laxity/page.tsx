...
export default function page() {
  const { data, isLoading } = useConditionTreatedDetailsQuery("skin-laxity");

  const sectionBars = [
    { id: 1, path: "Morpheus8", label: "Morpheus8" },
    { id: 1, path: "BodyTite", label: "BodyTite" },
    { id: 2, path: "FaceTite", label: "FaceTite" },
    { id: 3, path: " AccuTite", label: " AccuTite" },
    { id: 4, path: "OpusPlasma", label: "Opus Plasma" },
        ...
        <>
          ...
          <div id="BodyTite">
            <DynamicCardSectionPink
              image={`${process.env.NEXT_PUBLIC_ASSET_URL}/${data?.data?.subConditionTreats?.sub_condition_treat_details?.bodytite_image}`}
              title={
                data?.data?.subConditionTreats?.sub_condition_treat_details
                  ?.bodytite_title
              }
          ...
          <div id="FaceTite">
            <DynamicCardSection
              image={`${process.env.NEXT_PUBLIC_ASSET_URL}/${data?.data?.subConditionTreats?.sub_condition_treat_details?.facetite_image}`}
              title={
                data?.data?.subConditionTreats?.sub_condition_treat_details
                  ?.facetite_title
              }
              description={
                data?.data?.subConditionTreats?.sub_condition_treat_details
                  ?.facetite_description
              }
            />
          </div>
          <div id="AccuTite">
            <DynamicCardSectionPink
              image={`${process.env.NEXT_PUBLIC_ASSET_URL}/${data?.data?.subConditionTreats?.sub_condition_treat_details?.accutite_image}`}
              title={
                data?.data?.subConditionTreats?.sub_condition_treat_details
                  ?.accutite_title
              }
