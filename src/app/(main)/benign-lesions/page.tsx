...
export default function Page() {
  const { data, isLoading, error } =
    useConditionTreatedDetailsQuery("benign-lesions");
  ...
  const heading = safe(svc?.name) || safe(svc?.title) || "Skin Tags";
  ...
  const tabs = [
    {
      ...
      enabled:
        !!safe(det?.skin_tag_removal_title) ||
        !!safe(det?.skin_tag_removal_description) ||
        !!safe(det?.skin_tag_removal_image),
    },
    {
      id: 2,
      path: "CO2",
  ...
  return (
    <>
      ...
      {(safe(det?.skin_tag_removal_title) ||
        safe(det?.skin_tag_removal_description) ||
        safe(det?.skin_tag_removal_image)) && (
        <div id="SkinTagRemoval">
          <DynamicCardSection
            image={url(det?.skin_tag_removal_image) || FallbackImg.src}
            title={
              safe(det?.skin_tag_removal_title) ||
              "Say Goodbye to Skin Tags with Advanced Treatments"
            }
            description={
              safe(det?.skin_tag_removal_description) ||
              "Skin tags are benign growths often found where skin rubs together. In-clinic treatments provide safe, quick removal."
            }
